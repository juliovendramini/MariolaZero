...
class TCS:
    ...
    ID = 0x12
    CDATAL = 0x14
    def __init__(self, bus = None, portaMux = None):
        self.portaMux = portaMux
        self.bus = bus
        if self.bus is None:
            raise ValueError("O barramento I2C não pode ser None")
        if self.portaMux > 7 or self.portaMux < 0:
            raise ValueError("Canal inválido (deve ser 0 a 7)")
        self.select_channel()
            # Ativa o sensor (PON + AEN)
        bus.write_byte_data(self.TCS_ADDR, self.COMMAND_BIT | self.ENABLE, 0x03)

        # Tempo de integração (2.4ms × (256 - ATIME)) → ATIME = 0xC0 → ~60ms
        bus.write_byte_data(self.TCS_ADDR, self.COMMAND_BIT | self.ATIME, 0xC0)

        # Ganho (1x, 4x, 16x, 60x) → 0x01 = 4x
        bus.write_byte_data(self.TCS_ADDR, self.COMMAND_BIT | self.CONTROL, 0x01)
